...
对每个持仓，请从以下角度分析：
1. **盈亏状态**：当前盈亏幅度是否合理，是否触发止盈/止损信号
2. **仓位集中度**：该持仓占总资产的比例是否过高（>10% 需警告，>20% 需严重警告）
3. **资产类型适配**：该资产是否适合所在层级
4. **操作建议**：给出 买入(buy) / 卖出(sell) / 持有(hold) / 观望(watch) 之一
