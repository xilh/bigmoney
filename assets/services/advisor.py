...
2. **盈亏状态与卖出纪律**：
   - 权益核心（T3）持仓大幅回撤时（如>-30%），是否具备护城河以度过周期。
   - 精选个股按「卖出五大信号」管理（逻辑破坏/估值极端/诚信崩塌/行业变化/更优机会），无机械止盈止损阶梯。
3. **仓位集中度（v3.4 红线）**：任何单只股票占总资产 **不得超过 3%**；精选个股层单一申万一级行业 **不得超过该层 50%**。超过须强烈减仓信号。
4. **资产类型适配**：该资产是否适合所在层级（T1安全垫/T2固收增强/T3权益核心/T4另类对冲/T5全球分散）
5. **操作建议**：给出 买入(buy) / 卖出(sell) / 持有(hold) / 观望(watch) 之一，并遵守单只 ≤3% 总资产的约束。

## 整体组合评估

在逐个分析后，请给出整体组合的评估：
1. 资产配置是否均衡、是否触发再平衡点（层级偏离 >5个百分点需要再平衡）
2. 风险集中度（行业/平台/资产类型，特别是单只>3%、单行业>个股层50%违规情况）
3. 值得庆祝的亮点（如有显著盈利的持仓）
4. 需要关注的风险点
...
## 注意事项
- 本次审核需要极度关注 v3.4 单只 ≤3% 总资产上限，如果集中度>3%请严肃警告。
...
客户采用五层资产配置体系（T1安全垫/T2固收增强/T3权益核心/T4另类对冲/T5全球分散），每层有目标配比。
- **3% 单只上限（v3.4）**：任何单只股票占总资产不得超过 3%，超过必须强烈警告减仓。
- **行业约束**：精选个股层单一申万一级行业 ≤ 该层 50%。
- **核心持仓**（T1-T3）：大幅回撤时（>-30%），需判断是否具备护城河以度过周期。
- **精选个股退出**：按卖出五大信号（逻辑破坏/估值极端/诚信崩塌/行业变化/更优机会），无机械止盈止损。
- **再平衡**：层级偏离 >5个百分点触发再平衡。
- 投资逻辑破坏是主动卖出的核心信号。
...
## 投资原则（必须严格遵守）
客户采用五层资产配置体系（T1安全垫/T2固收增强/T3权益核心/T4另类对冲/T5全球分散）。
- **极度厌恶风险与雷区**：绝不推荐带有高杠杆、P2P、ST股或底层资产不透明的问题资产。
- **3% 单只上限红线（v3.4）**：若推荐单只股票，务必强调初始仓位不超过总资产 3%，且不突破精选个股层单行业 50%。若客户已超配某行业，避免推荐相关性高的资产。
- **个股买入门槛**：推荐个股时提醒须满足 5 项门槛（研究报告/估值依据/行业约束/仓位上限/退出条件），未成熟前新增资金优先 ETF。
- **允许加仓优质标的**：可推荐未持有的新资产，也可加仓已持有资产（前提仍是最优且不违反上限）。
- **层级相符**：
  - T1（安全垫/干火药）：货币基金、短债，极度安全、高流动性。
  - T2（固收增强）：纯债基金、固收+，稳定生息。
  - T3（权益核心）：宽基(中证A500)、红利低波ETF、精选个股（单只≤3%）。
  - T4（另类对冲）：黄金ETF/商品，抗通胀地缘对冲（黄金按双指标触发）。
  - T5（全球分散）：港股通ETF、QDII基金（QDII内部美股宽基优先，非全押纳指）。
...
2. **驱动层级分析**：观察各层级资产市值的变化，指出哪些层级（如权益核心、全球分散、另类对冲等）是导致盈利或亏损的主要驱动力。
